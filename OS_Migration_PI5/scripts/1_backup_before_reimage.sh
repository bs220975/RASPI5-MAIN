#!/bin/bash
# =============================================================
# PRE-REIMAGE BACKUP SCRIPT — Raspberry Pi 5
...
GDRIVE_DEST="gdrive:/pi5_backups/${BACKUP_NAME}"
LOCAL_STAGING="/tmp/pi5_backup_staging/${BACKUP_NAME}"

echo -e "${CYAN}============================================${NC}"
echo -e "${CYAN}   Pi5 Pre-Reimage Backup Script${NC}"
echo -e "${CYAN}   $(date)${NC}"
echo -e "${CYAN}============================================${NC}"
echo ""
...
ITEMS=(
    /home/pi5/pi5_drive
    /home/pi5/python_raspi
    /home/pi5/.ssh
    /home/pi5/.bashrc
    /home/pi5/.bash_aliases
    /home/pi5/.config/rclone/rclone.conf
    /etc/mosquitto
)
...
crontab -l > "$LOCAL_STAGING/crontab/crontab_pi5.txt" 2>/dev/null || echo "# empty" > "$LOCAL_STAGING/crontab/crontab_pi5.txt"
...
cp -r /home/pi5/.ssh/. "$LOCAL_STAGING/ssh/" 2>/dev/null || true
echo -e "${GREEN}  Done${NC}"
...
cp /home/pi5/.bashrc          "$LOCAL_STAGING/configs/bashrc"          2>/dev/null || true
cp /home/pi5/.bash_aliases    "$LOCAL_STAGING/configs/.bash_aliases"   2>/dev/null || true
# rclone.conf — needed to restore Drive auth on new Pi
cp /home/pi5/.config/rclone/rclone.conf "$LOCAL_STAGING/configs/rclone.conf" 2>/dev/null || true
cp /home/pi5/pi5_drive/Git_projects/RASPI5-MAIN/config.py "$LOCAL_STAGING/configs/config.py" 2>/dev/null || true
...
rsync -a --exclude='.git' /home/pi5/pi5_drive/ "$LOCAL_STAGING/code/pi5_drive/"
rsync -a /home/pi5/python_raspi/ "$LOCAL_STAGING/code/python_raspi/" 2>/dev/null || true
echo -e "${GREEN}  Done${NC}"
...
if [ "$USE_GDRIVE" = true ]; then
    ...
    echo -e "${GREEN}  gdrive:/pi5_backups/${BACKUP_NAME}${NC}"
    echo -e "${GREEN}============================================${NC}"
    echo ""
    echo -e "${YELLOW}On new Pi5, post-install script will find this at:${NC}"
    echo -e "  gdrive:/pi5_backups/${BACKUP_NAME}"
    # Clean up staging
    rm -rf /tmp/pi5_backup_staging
else
    echo ""
    echo -e "${YELLOW}============================================${NC}"
    ...
    echo -e "  cp -r \"$LOCAL_STAGING\" /media/pi5/YOUR_USB/"
fi
