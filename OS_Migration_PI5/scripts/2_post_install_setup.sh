...
# Raspberry Pi OS 64-bit (Bookworm) | Pi5
# hostname: pi5 | user: pi5 | IP: 192.168.1.108
# =============================================================
# Usage (SSH into fresh Pi5, then run):
...
REPO_MAIN="RASPI5-MAIN"
PI5_DRIVE_DIR="/home/pi5/pi5_drive"
MYENV_DIR="/home/pi5/myenv"
LOG_FILE="/home/pi5/post_install_$(date +%Y-%m-%d_%H-%M).log"

log() { echo -e "$1" | tee -a "$LOG_FILE"; }
ask() { echo -e "${YELLOW}$1${NC}"; read -p "(y/n): " _ans; [[ "$_ans" == "y" || "$_ans" == "Y" ]]; }

log "${CYAN}============================================${NC}"
log "${CYAN}   Pi5 Post-Install Setup Script${NC}"
log "${CYAN}   $(date)${NC}"
log "${CYAN}============================================${NC}"
log ""
...
# ─────────────────────────────────────────────
# STEP 5 — Restore pi5_drive from Drive  [USER INPUT: y/n]
# ─────────────────────────────────────────────
log ""
log "${BLUE}[STEP 5/12] Restoring pi5_drive from Google Drive...${NC}"
mkdir -p "$PI5_DRIVE_DIR"

GDRIVE_BACKUP_DIR=""
if rclone lsd gdrive:/pi5_backups &>/dev/null 2>&1; then
    GDRIVE_BACKUP_DIR=$(rclone lsd gdrive:/pi5_backups/ 2>/dev/null | awk '{print $NF}' | grep "pre_reimage_" | sort -r | head -1)
fi

if [ -n "$GDRIVE_BACKUP_DIR" ]; then
    log "${GREEN}  Found backup: gdrive:/pi5_backups/${GDRIVE_BACKUP_DIR}${NC}"
    if ask "  ► Restore from gdrive:/pi5_backups/${GDRIVE_BACKUP_DIR}?"; then
        rclone copy "gdrive:/pi5_backups/${GDRIVE_BACKUP_DIR}/code/pi5_drive" "$PI5_DRIVE_DIR" --progress 2>&1 | tee -a "$LOG_FILE"
        # Restore rclone.conf from Drive backup if not from USB
        [ "$RCLONE_RESTORED" = false ] && \
            rclone copy "gdrive:/pi5_backups/${GDRIVE_BACKUP_DIR}/configs/rclone.conf" /home/pi5/.config/rclone/ 2>/dev/null || true
        # Restore crontab
        rclone copy "gdrive:/pi5_backups/${GDRIVE_BACKUP_DIR}/crontab/crontab_pi5.txt" /tmp/ 2>/dev/null && \
            crontab /tmp/crontab_pi5.txt 2>/dev/null && log "${GREEN}  Crontab restored${NC}" || true
        log "${GREEN}  Backup restored from Google Drive${NC}"
    fi
fi

if rclone lsd gdrive:/pi5_drive &>/dev/null 2>&1; then
    if ask "  ► Also sync gdrive:/pi5_drive → /home/pi5/pi5_drive?"; then
        rclone sync gdrive:/pi5_drive "$PI5_DRIVE_DIR" --exclude=".git/**" --exclude=".lgd-nfy0" --progress 2>&1 | tee -a "$LOG_FILE"
        log "${GREEN}  pi5_drive synced from Google Drive${NC}"
    fi
fi

mkdir -p "$PI5_DRIVE_DIR/Git_projects"
log "${GREEN}  pi5_drive folder structure ready${NC}"

# ─────────────────────────────────────────────
# STEP 6 — Clone RASPI5-MAIN from GitHub
# ─────────────────────────────────────────────
log ""
log "${BLUE}[STEP 6/12] Cloning RASPI5-MAIN from GitHub...${NC}"
if [ ! -d "$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/.git" ]; then
    gh repo clone ${GITHUB_USER}/${REPO_MAIN} "$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN"
    log "${GREEN}  Cloned RASPI5-MAIN${NC}"
else
    git -C "$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN" pull
    log "${GREEN}  RASPI5-MAIN up to date${NC}"
fi

mkdir -p "$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/logs"
...
MYENV_REQ="$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/OS_Migration_PI5/configs/pip_myenv.txt"
FALLBACK_REQ="$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/requirements.txt"
...
if [ -f "/home/pi5/${INFLUX_PKG}" ]; then
    cp "/home/pi5/${INFLUX_PKG}" /tmp/
else
...
MOSQ_CONF_BACKUP="$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/OS_Migration_PI5/configs/mosquitto"
...
SERVICES_SRC="$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/OS_Migration_PI5/services"
mkdir -p "$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/logs"
...
CERTS_DEST="$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/aws_certs"

if [ -d "$CERTS_DEST" ] && [ "$(ls -A $CERTS_DEST 2>/dev/null)" ]; then
    log "${GREEN}  AWS certs already in repo${NC}"
elif [ -n "$GDRIVE_BACKUP_DIR" ]; then
    mkdir -p "$CERTS_DEST"
    rclone copy "gdrive:/pi5_backups/${GDRIVE_BACKUP_DIR}/aws_certs" "$CERTS_DEST" 2>/dev/null && \
...
BASHRC_BACKUP="$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/OS_Migration_PI5/configs/bashrc.txt"
ALIASES_BACKUP="$PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/OS_Migration_PI5/configs/.bash_aliases"

[ -f "$BASHRC_BACKUP" ]  && cp "$BASHRC_BACKUP"  /home/pi5/.bashrc       && log "${GREEN}  .bashrc restored${NC}"
[ -f "$ALIASES_BACKUP" ] && cp "$ALIASES_BACKUP" /home/pi5/.bash_aliases  && log "${GREEN}  .bash_aliases restored${NC}"

grep -q "Git_projects/RASPI5-MAIN/shell_scripts" /home/pi5/.bashrc || \
    echo 'export PATH="$PATH:/home/pi5/pi5_drive/Git_projects/RASPI5-MAIN/shell_scripts"' >> /home/pi5/.bashrc
...
log "  2. Verify setup:    bash $PI5_DRIVE_DIR/Git_projects/RASPI5-MAIN/OS_Migration_PI5/scripts/3_verify_setup.sh"
